		...
		<div v-for="(item, index) in buttons" :key="index">
			<div class="flex-center add-content" :class="[ addButtonType == 'menuButton' ? '' : 'add-fixed-button' ]">
				<div class="flex-center" style="width: 100%; justify-content: space-between;">
					<!--<Select v-model="item.type" @on-change="handleChange($event, index)" :transfer="true" style="width:120px; margin-right: 15px; flex-shrink: 0;" size="small">
					...
					<!-- <Select v-model="item.type" @on-change="handleChange($event, index)" :transfer="true" :class="[lineBreak ? 'line-break' : 'no-line-break' ]" size="small">
						<Option v-for="item in cityList" :value="item.value" :key="item.value">{{ item.label }}</Option>
					</Select> -->
					<!--回复按钮的需要引用-->
					<!--<div v-if="item.type == 'reply' || item.type == 'openApp' || item.type == 'sendAddress' || item.type == 'bringUp' || item.type == 'addressLocation' " class="txt-limit-div" style="position: relative; width: 100%; padding-right: 16px;">
					...
					<i @click="removeList(index)" class="cursor-pointer el-icon-delete" style="font-size:16px;color:#515a6e;"></i>
				</div>
				<!--换行展示-->
				<template v-if="lineBreak">
						<!--新增引用内容-->
				...
				<template v-if="item.type == 'url'">
					<div class="txt-limit-div line-break-one"  :class="[pageParameters.showQuote ? 'line-break-one-new' : '' ]">
						<roInputAboutQuoteOne :inputAboutQuoteOne="item"  quoteKey='originalContent' :nodeList="nodeList" :otherNodeList="otherNodeList" :nowNode="nowNode" size="small" placeholder="链接需以http(s)://开头"   :showQuote="pageParameters.showQuote"></roInputAboutQuoteOne>
					</div>
				</template>
