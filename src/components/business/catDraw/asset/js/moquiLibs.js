var moqui = {
  ...
  notifyGrowl: function(jsonObj) {
    if (!jsonObj) return;
    if (moquiVue) {
      moqui.handleNotice(jsonObj.title, jsonObj.type);
    }
  },
...
moqui.handleNotice = function(msg, type) {
  switch (type) {
    case 'info':
      moquiVue.$Notice.info({
        title: "提醒",
        desc: msg || "操作成功!"
      })
      break;
    case 'success':
      moquiVue.$Notice.success({
        title: "成功",
        desc: msg || "操作成功!"
      })
      break;
    case 'warning':
      moquiVue.$Notice.warning({
        title: "警告",
        desc: msg || "操作成功!"
      })
      break;
    case 'error':
      moquiVue.$Notice.error({
        title: "错误",
        desc: msg || "操作失败!"
      })
      break;
    ...
    if (moqui.isArray(messages)) {
      for (var mi = 0; mi < messages.length; mi++) {
        ...
        if (moqui.isPlainObject(messageItem)) {
          ...
          moqui.handleNotice(messageItem.message, msgType);
        } else {
          moqui.handleNotice(messageItem, 'info');
        }
        notified = true;
      }
    } else {
      moqui.handleNotice(messages, 'info');
      notified = true;
    }
  }
  if (errors) {
    if (moqui.isArray(errors)) {
      for (var ei = 0; ei < errors.length; ei++) {
        moqui.handleNotice(errors[ei], 'danger');
        notified = true;
      }
    } else {
      moqui.handleNotice(errors, 'danger');
      notified = true;
    }
  }
  if (validationErrors) {
...
moqui.notifyValidationError = function(valError) {
  ...
  moqui.handleNotice(message, 'danger');
};
moqui.handleAjaxError = function(jqXHR, textStatus, errorThrown) {
  ...
  if (jqXHR.status === 401) {
    ...
    if (errorThrown.indexOf('abort') < 0) {
      ...
      moqui.handleNotice(msg, 'danger');
    }
  } else if (!notified) {
    var errMsg = 'Error: ' + errorThrown + ' (' + textStatus + ')';
    moqui.handleNotice(errMsg, 'danger');
  }
}
