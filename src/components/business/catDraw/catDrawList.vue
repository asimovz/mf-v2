	...
	<div :style="listStyle">
		<div class="default-onceReplyModeLabel">
			<div v-if="!hiddenModeLabel" class="flex-center onceReplyModeLabel-radio">
				...
				<!-- <RadioGroup v-model="eventList.onceReplyModeLabel" @on-change="setOnceReplyMode" >
					<Radio v-for="(val,key,index) in replys" :key="index" :label="val"></Radio>
				</RadioGroup> -->
			</div>
			
			<!--新增顶部的添加按钮-->
			<catDrawListAddMenu v-if="selectOnlyOne"  @addMenu="addMenu" :selectOnlyOne="selectOnlyOne" :selectOnlyOneObj="eventList.inputContents && eventList.inputContents[0]"></catDrawListAddMenu>
			...
			<div v-for="(item, index) in eventList.inputContents" :key="index" class="instance">
				...
				<!--图片，音频和视频上传-->
				<catDrawListMedia :item="item" v-if="item.replyType == 'image' || item.replyType == 'voice' || item.replyType == 'video'" :acceptUrl="acceptUrl" :pageParameters="pageParameters"></catDrawListMedia>
				<!--新增单卡-->
