...
export default {
  ...
  beforeMount() {
    ...
    if(this.params.hasOwnProperty("data")){
      ...
      this.href = param.length > 0 ? this.params.props.transitionUrl + '?' + param.join('&') : this.params.props.transitionUrl + "?" + this.params.colDef.field + "=" + this.params.value
  ...
  methods: {
      ...
      go: function (event) {
        ...
        if (this.loadId && this.loadId.length > 0) {
          this.params.context.componentParent.$root.loadContainer(this.loadId, this.href)
        } else if(this.targetDialog && this.targetDialog.length > 0){
          this.params.context.componentParent.$root.eventBus.$emit("dynamic_visible_change_"+this.targetDialog,{
            href: this.href,
            title: this.linkText,
          });
        }else {
          if (event.ctrlKey || event.metaKey) {
            ...
            this.params.context.componentParent.$root.setUrl(this.linkHref)
            this.params.context.componentParent.$root.eventBus.$emit("updateAsideMenu",true,this.linkHref)
  ...
  computed: {
    linkHref: function () {
      return this.params.context.componentParent.$root.getLinkPath(this.href)
    },
