<template>
  <div style="display:inline-block">
    ...
    <template v-if="type=='button'">
      <Button v-show="multiple || !fileList.length" name="filebtn" type="primary" ghost icon="ios-cloud-upload-outline" @click="handleClick">{{text}}</Button>

      <template v-for="(file, index) in fileName">
        <span class="components_upload_list">{{file}} <Icon type="close" @click.native="handleRemove(index)"></Icon></span>
      </template>
    </template>
    <template v-else>
      ...
      <template v-if="fileList.length">
        <div class="previews" v-for="(file, index) in fileList" :style="styles">
          <img :src="file" ref="uploadImg" :style="imgStyle">
          <span class="upload_files-actions" v-if="type === 'card'">
            <Icon type="close" size="16" @click.native="handleRemove(index)"></Icon>
            <Icon type="search" size="16" @click.native="handleScale(index)"></Icon>
          </span>
        </div>
      </template>
    </template>
