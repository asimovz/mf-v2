		...
		<div ref="selectBox" :class="['aw-tree-select','ivu-select-selection', {active: showClass.isActive}]" tabindex="-1"  @click="focus">
			<div class="tag-box">
				...
				<div v-else>
					...
					<el-tag size="small" :key="index" :name="JSON.stringify(item)" closable @close="handleTagDel(index)"> {{item.title || item.text ||item.label}}
						</el-tag>
					</template>
				</div>
			</div>
			
			<input type="hidden" :name="name" :form="form" v-model="selectValues" v-validate="validate"
				:data-vv-as="fieldTitle" :data-vv-name="name"/>
			
			<div class="clear-btn">
				<i :class="this.showClass.isActive ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"></i>
			</div>
  ...
  watch: {
	...
	methods: {
		...
		setContentPos() {
			...
			let top = posArr.top + this.$refs.selectBox.clientHeight + scrollHeight
