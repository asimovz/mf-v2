  ...
  <div style="margin:10px">
    <div class="search-area" style="margin-bottom: 10px" v-if="searchable">
      <m-input
        ...
        icon="search"
        placeholder="搜索"
        @on-input-change="searchTreeUpdate"
      ></m-input>
    </div>
    <el-tree
      :data="data"
...
export default {
  ...
  mounted() {
    ...
    if (this.dependsOn) {
      ...
      for (let doParm in dependsOnMap) {
        this.$root.eventBus.$on(
          dependsOnMap[doParm] + "_value_change",
          result => {
            let param = {};
    ...
    let height =
      this.maxHeight == 0 ? bodyHeight - tree.top - 30 : this.maxHeight;
    this.styles = {
      overflow: "auto",
  ...
  methods: {
    ...
    toTree(data, expands, expandAll, selected) {
      ...
      let that = this;
			...
			// !! el组件自带默认展开的参数，不用遍历所有数据加上expand
      that.defaultExpanded = expands;
			that.defaultChecked = [];
      data.forEach(function(item) {
        ...
        if (that.showCheckbox) {
          if (selected.indexOf(item.nodeId) !== -1) {
            item.checked = true;
            that.defaultChecked.push(item.nodeId)
            
          }
        }
        if (item.f == expands[expands.length - 1]) {
          item.selected = true;
        }
    ...
    selectChange(item) {
      ...
      if (!item || !item[0] || !item[0].bizData) return false;
    ...
    checkChange(data,obj) {
      ...
      nodes.forEach(({ nodeId, label, parentNodeId }) => {
    ...
    _loadData() {
      ...
      this.$http.get(this.items, { params: this.parameters }).then(res => {
        ...
        let selectTreeItem = res.data.filter(item => {
          if (
            this.initCheckedNodes.includes(item.nodeId) &&
            item.parentNodeId
          ) {
    ...
    filterNode(value, data) {
      ...
      return this.searchTree(data, value);
    },
    searchTree: function(data, searchKey) {
      ...
      let func = function(val, index) {
        if (val.label) {
          let itemValue = val.label.toLowerCase();
          if (!val.children || val.children.length == 0) {
            // 优化搜索，包含与被包含的情况均考虑
            if (
              itemValue.includes(searchKey) ||
              searchKey.includes(itemValue)
            ) {
              isExpand = true;
              val.selected = true;
            } else {
            ...
            val.selected =
              itemValue.includes(searchKey) || searchKey.includes(itemValue);
