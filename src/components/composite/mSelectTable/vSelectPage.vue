<template>
    <div>
        ...
        <div :class="{'sp-input-container':true,'sp-open':show,'sp-disabled':disabled}" ref="caller" @click="open">
            ...
            <div :class="{'sp-button':true,open: show}"><span class="sp-caret el-icon-arrow-down"></span></div>
        </div>
        ...
        <v-dropdown ref="dropdown" @show-change="dropdownVisible" >
            ...
            <div class="sp-result-area" v-show="!message" ref="list">
                ...
                <table v-if="Array.isArray(tbColumns)&&tbColumns.length" ref="table" v-show="list.length" class="sp-table">
                    <thead>
                        <tr>
            ...
            <div class="sp-pagination" v-show="!message" v-if="pagination">
                ...
                <ul ref="page">
                    ...
                    <li :class="{'sp-disabled':pageNumber===totalPage || totalRows===0}" class="sp-right" :title="i18n.last">
                        <a href="javascript:void(0);" @click="switchPage('last')" >
                            <i class="sp-iconfont sp-icon-last"></i>
                        </a>
                    </li>
                    <li :class="{'sp-disabled':(pageNumber===totalPage || totalRows===0)}" class="sp-right" :title="i18n.next">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            includesObj(arr,obj) {
                ...
                for(var i=0;i < arr.length;i++){
                    if(arr[i][this.keyField] == obj[this.keyField]) {
                        isInclude = i
                        break
                    }
                }
            ...
            pickPage(check){
                ...
                for(let row of this.list){
                    if(check) {
                        if(this.includesObj(this.picked,row) == -1 && (this.maxSelectLimit == 0 || this.picked.length < this.maxSelectLimit)) {
                            this.picked.push(row);//picked current page items
                        }
                    } else {
                        let idx = this.includesObj(this.picked,row)
                       if(idx > -1){//removed current page items
            ...
            selectItem(row){
                if(typeof row == "object") {
                    if(this.picked.findIndex(val=>val[this.keyField] === row[this.keyField])>-1) return
                } else {
            ...
            remote(init,dynamicParams){
                if(typeof(this.data) === 'string' && this.dataLoad && typeof(this.dataLoad === 'function')){
                    ...
                    if(dynamicParams){
                        ...
                        queryParams = Object.assign(queryParams,this.dynamicParams)
                    }
                            ...
                            if(!that.resultFormat || typeof(that.resultFormat) !== 'function'){
                                ...
                                if(tmpObj && Object.keys(tmpObj).length){
                                    if(!init){
                                        ...
                                        if(arr.length){
                                            let matchRows = tmpObj.list.filter(val=>arr.includes(String(val[that.keyField])));
                                            if(matchRows.length) {
            ...
            switchPage(pNum){
                ...
                this.populate(this.dynamicParams);
            },
            initSelection(){
                if(this.value) {
                    if(Array.isArray(this.data)){
        ...
        watch: {
            ...
            value(val){
                if(!val.trim().length) this.picked = []
                // this.initSelection();
            },
            data(val){
                this.sortList();
...
div.v-selectpage{
    ...
    div.sp-input-container {
        position: relative;
        span.sp-placeholder { color: #c9cdd2;}
        &.sp-open{
            div.sp-input, div.sp-inputs {
...
div.sp-pagination {
    ...
    text-align: center;
    font-size: 12px;
    div.sp-page-info { height: 23px;line-height: 25px;font-weight: 400; }
    ul {
        position: absolute;
        top: 0;
        padding: 0;
